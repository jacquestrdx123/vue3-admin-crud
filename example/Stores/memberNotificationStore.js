...
export const useMemberNotificationStore = defineStore('memberNotifications', {
  state: () => ({
    notifications: [],
    unreadCount: 0,
    isLoading: false,
  ...
  actions: {
    ...
    loadSeenIds() {
      try {
        const stored = localStorage.getItem('seen_member_notification_ids')
        if (stored) {
    ...
    saveSeenIds() {
      try {
        localStorage.setItem('seen_member_notification_ids', JSON.stringify(this.seenNotificationIds))
      } catch (error) {
    ...
    async fetchUnreadNotifications() {
      try {
        this.isLoading = true
        this.error = null
        const response = await axios.get('/api/member/notifications/unread')
    ...
    async fetchUnreadCount() {
      try {
        const response = await axios.get('/api/member/notifications/unread-count')
    ...
    async markAsRead(notificationId) {
      try {
        await axios.patch(`/api/member/notifications/${notificationId}/mark-read`)
    ...
    async markAllAsRead() {
      try {
        await axios.patch('/api/member/notifications/mark-all-read')
    ...
    async deleteNotification(notificationId) {
      try {
        await axios.delete(`/api/member/notifications/${notificationId}`)
    ...
    async deleteAllRead() {
      try {
        await axios.delete('/api/member/notifications/delete-all-read')
