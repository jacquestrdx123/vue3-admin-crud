...
onMounted(async () => {
  try {
    const response = await axios.get(route('vue.memberships.options', { type: 'external-companies' }))
...
const submitForm = async () => {
  ...
  try {
    const routeName = editingReward.value
      ? 'vue.memberships.rewards.update'
      : 'vue.memberships.rewards.store'
    
    const routeParams = editingReward.value
      ? { id: props.membershipId, rewardId: props.reward.id }
