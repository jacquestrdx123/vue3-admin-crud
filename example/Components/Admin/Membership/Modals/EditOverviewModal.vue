<template>
  <Modal :show="show" title="Edit Membership Overview" max-width="2xl" @close="$emit('close')">
    ...
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Membership Name *</label>
          <input
...
const formData = reactive({
  ...
  abbreviation: props.membership.abbreviation || '',
  monthly_price: props.membership.monthly_price || 0,
  quarterly_price: props.membership.quarterly_price || 0,
  yearly_price: props.membership.yearly_price || 0,
  initial_fee: props.membership.initial_fee || 0,
  rank: props.membership.rank || 0,
  show_on_application_screen: props.membership.show_on_application_screen ?? true,
...
watch(() => props.membership, (newMembership) => {
  if (newMembership) {
    Object.assign(formData, {
      ...
      abbreviation: newMembership.abbreviation || '',
      monthly_price: newMembership.monthly_price || 0,
      quarterly_price: newMembership.quarterly_price || 0,
      yearly_price: newMembership.yearly_price || 0,
      initial_fee: newMembership.initial_fee || 0,
      rank: newMembership.rank || 0,
      show_on_application_screen: newMembership.show_on_application_screen ?? true,
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.put(route('vue.memberships.update-overview', props.membership.id), formData)
    ...
    if (error.response?.data?.errors) {
      ...
      errorMessage.value = error.response?.data?.message || 'Failed to update membership overview'
    }
  } finally {
