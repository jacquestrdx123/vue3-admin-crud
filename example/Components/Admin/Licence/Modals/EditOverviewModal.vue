<template>
  <Modal :show="show" title="Edit Licence Overview" max-width="2xl" @close="$emit('close')">
    ...
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Licence Name *</label>
          <input
...
const formData = reactive({
  ...
  abbreviation: props.licence.abbreviation || '',
  monthly_price: props.licence.monthly_price || 0,
  quarterly_price: props.licence.quarterly_price || 0,
  yearly_price: props.licence.yearly_price || 0,
  initial_fee: props.licence.initial_fee || 0,
  rank: props.licence.rank || 0,
  show_on_application_screen: props.licence.show_on_application_screen ?? true,
  description: props.licence.description || ''
})

watch(() => props.licence, (newLicence) => {
  if (newLicence) {
    Object.assign(formData, {
      name: newLicence.name || '',
      abbreviation: newLicence.abbreviation || '',
      monthly_price: newLicence.monthly_price || 0,
      quarterly_price: newLicence.quarterly_price || 0,
      yearly_price: newLicence.yearly_price || 0,
      initial_fee: newLicence.initial_fee || 0,
      rank: newLicence.rank || 0,
      show_on_application_screen: newLicence.show_on_application_screen ?? true,
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.put(route('vue.licences.update-overview', props.licence.id), formData)
    ...
    if (error.response?.data?.errors) {
      ...
      errorMessage.value = error.response?.data?.message || 'Failed to update licence overview'
    }
  } finally {
