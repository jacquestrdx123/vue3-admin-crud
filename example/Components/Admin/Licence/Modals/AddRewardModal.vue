...
onMounted(async () => {
  try {
    const response = await axios.get(route('vue.licences.options', { type: 'external-companies' }))
...
const submitForm = async () => {
  ...
  try {
    const routeName = editingReward.value
      ? 'vue.licences.rewards.update'
      : 'vue.licences.rewards.store'
    
    const routeParams = editingReward.value
      ? { id: props.licenceId, rewardId: props.reward.id }
      : { id: props.licenceId }
